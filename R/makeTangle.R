...
# 	datadir <- '/data'
# 	setwd(wkdir)
# 	load(file=paste(wkdir,datadir,'/egsightings.rdata',sep=""))
	...
	# tangled[tangled$EGNo==1045,'EndDate'] <- ymd('1935-03-24') # this is because 
	# tangled$interval <- new_interval(tangled$EndDate,tangled$StartDate)
	# tangled$duration <- rep(NA,nrow(tangled))
	# tangled$duration <- as.duration(tangled$interval)
	# tangled$period <- as.period(tangled$interval)
	# tangled$months <- tangled$interval %/% months(1)
