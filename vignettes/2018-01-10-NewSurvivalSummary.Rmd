...
For the paper, we also excluded animals that were ship struck, or had severe prop scars. Here I also add in the known dead animals. Whether or not to include/exlcude these is a point of discussion. 
...
### Summary of Survival
For each animal's last entanglement, we can now build up a data structure containing the two $Y$ distributions.
...
for (i in seq_along(tangSub$EGNo)){
  ...
  yrefMat[[i]] <- colMeans(refparams, na.rm = TRUE)
  
  # # plot out each animal for reference
  # pdat <- data.frame(t(refparams))
  # colnames(pdat) <- refAns
  # pdat$time <- seq_along(range1)
  # 
  ...
  #   geom_line()+
  #   facet_wrap(~ variable)+
  #   labs(x = 'Months Since End of Last Entanglement', y = 'Probability of Survival')+
  #   ggtitle('Survival of Reference Animals', subtitle = paste0('For Entangled Whale: ', refAns[j]))
  # 
  # pdf(file = 'E:/rob/Documents/research/projects/rightWhaleEntanglement/documents/images/candEnt-vs-refEnt.pdf', width = 10, height = 6.1)
  #   print(p)
  # dev.off()
...
ggplot(meltdf, aes(x = time, y = value)) +
    ...
    facet_wrap(~ variable)+
    labs(x = 'Months Since End of Last Entanglement', y = 'Probability of Survival')+
    ggtitle('Survival of Reference Animals', subtitle = paste0('For Entangled Whale: ', tangSub$EGNo[i]))
```

And we can summarize the survival for this reference group as well:

```{r}
plot(plotyref, type = 'l', ylim = c(0, 1), xlab = 'Months Since Entanglement', ylab = 'Mean Probability of Survival\n Among Reference Animals')
...
We'll demonstrate the algorithm for one individual. Here we sample from $Y_e$ and $Y_c$ where the $e$ and $c$ subscripts refer to the entangled animals and the candidate, or reference, animals. In the model from Schick et al. 2013, survival was distributed as a Binomial. To account for the uncertainty in survival, we sample from the posterior using the probabilities for the entangled and the reference animal:

```{r}
entprobs <- tangSub$svals[[1]]
refprobs <- yrefMat[[1]]
...
Now we examine the differences ($Z$) for this animal, which shows an approximately :

```{r}
sdiff <- data.frame(refAn = tangSub$EGNo[1], meanDiff = survMat - cansurvMat)
hist(sdiff$meanDiff, breaks = 20, col = 'cornsilk', border = 'lightgrey', xlab = "Difference in Survival Probability", main = '')
```
...
for(i in seq_along(tangSub$EGNo)){
  ...
  sdiff <- data.frame(refAn = tangSub$EGNo[i], meanDiff = colMeans(survMat) - colMeans(cansurvMat), 
                      gender = gidx, gearStatus = tangSub$gear[i], injuryClass = tangSub$gearInj[i],
