...
```{r}
sdiff <- data.frame(refAn = tangSub$EGNo[1], meanDiff = rowMeans(survMat - cansurvMat))
hist(sdiff, breaks = 30, col = 'cornsilk', border = 'lightgrey')
```
...
for(i in seq_along(tangSub$EGNo)){
  ...
  sdiff <- data.frame(refAn = tangSub$EGNo[i], meanDiff = rowMeans(survMat - cansurvMat), 
                      gender = tangSub$EGNo[i], gearStatus = tangSub$EGNo[i], injuryClass = tangSub$EGNo[i])
