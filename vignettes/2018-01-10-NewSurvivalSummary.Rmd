...
for (i in seq_along(tangSub$EGNo)){
  ...
  for (j in seq_along(refAns)){
    ...
    # # quick plot check for one animal
    ...
    # ggplot(meltdf, aes(x = time, y = value, group = variable, color = variable)) +
    #   geom_line()+
    #   scale_color_discrete(aes(color = variable))+
    #   facet_wrap(~ variable)
  }
  yrefMat[[i]] <- colMeans(refparams, na.rm = TRUE)
  
  # Plot average survival for reference animals
  # plot(yrefMat[[i]], type = 'l', ylim = c(0, 1))
  # theta <- yrefMat[[i]] * (1 - yrefMat[[i]])
  # lines(yrefMat[[i]] + theta, lty = 2)
  # lines(yrefMat[[i]] - theta, lty = 2)
}
