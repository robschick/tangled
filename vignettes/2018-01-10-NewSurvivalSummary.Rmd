...
What does this look like? We start by choosing one entangled animal and examining it's health trace:
...
2. From that we gather the stop times (in particular, we'll make use of the ```ewindmonyr``` time indices to pull out the moments)
3. Calculate the survival probability from the end of entanglement until presumed death
...
### Reference Animal
Ok, with that built for the entangled animal, we want to sample the reference animals - against which we'll compare survival. In practice that means all animals who are alive for the whole duration of the window and who are not entangled during that interval. We'll calculate the mean of all those animals for each of the months during the entanglement survival window. Let's say we have a an 8-month survival window when the entanglement declines from 1 to 0. During each of those months, we'll calculate the difference between the individual animal and the reference class. Each of these animals will have metadata that indicates injury and gender. By building up the distribution of differences we will be able to quantify the impact of entanglement on survival by category.
...
Next, we find the reference animmals, and as above, build up the survival probabilities. Then we can subtract them to get the distribution of differences. 