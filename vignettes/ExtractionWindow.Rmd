...
p
ggsave(plot = p, filename = 'windowDiagram.pdf', path = '/Users/robs/Dropbox/Papers/KnowltonEtAl_Entanglement/images/', device = 'pdf')
ggsave(plot = p, filename = 'windowDiagram.png', path = '/Users/robs/Dropbox/Papers/KnowltonEtAl_Entanglement/images/', device = 'png', dpi = 300, width = 9, height = 6, units = 'in', scale = 0.8)  
```
...
p3 <- ggplot(hplotsub1, aes(x = date, y = hanom, fill = pos)) +
      ...
      annotate('text', x = as.Date(tdatplot$EndDateWindow[2]) - months(3), y = 18, size = textsize, label = '1')+ 
      annotate('point', x = as.Date(tdatplot$EndDateWindow[2]), y = 14.5, size = dotsize)+
      annotate('text', x = as.Date(tdatplot$EndDateWindow[2]), y = 18.5, size = textsize, label = '2a')+    
      annotate('point', x = as.Date(tdatplot$EndDateWindow[2] + months(12)), y = -8.5, size = dotsize)+
      annotate('text', x = as.Date(tdatplot$EndDateWindow[2] + months(12)), y = -12, size = textsize, label = '3')+     
      scale_x_date(minor_breaks = date_breaks(width = "1 year"))+
      lims(y = c(-22.5, 22.5))+
      theme(plot.margin = unit(c(0, 1, 0, 0.5), 'line'))+
      theme(axis.text.y = element_text(size = base_size, hjust=1),
...
#                ymin = -22, ymax = 22, alpha = 0.4, fill = pal[3])+
      ...
      annotate('text', label = '1', x = as.Date(tdatplot$StartDateWindow[4]) - months(3), y = -16, size = textsize)+
      annotate('text', label = '2a', x = as.Date(tdatplot$StartDateWindow[4]) + months(3), y = -13, size = textsize)+    
      annotate('text', label = '2b', x = as.Date(tdatplot$EndDateWindow[4]), y = 7.5, size = textsize)+
      annotate('text', label = '3', x = as.Date(tdatplot$recov12months[4]), y = 11.25, size = textsize)+
      scale_x_date(minor_breaks = date_breaks(width = "1 year"))+
      lims(y = c(-22.5, 22.5))+
...
```{r}
# Make panel d - as panel 3, but with just one point labeled.
...
# make panel e - as panel 4, but with two points labeled
p6 <- ggplot(hplotsub2, aes(x = date, y = hanom, fill = pos)) +
      ...
      annotate('rect', xmin = as.Date(tdatplot$StartDateWindow[4]), 
               xmax = as.Date(tdatplot$EndDateWindow[4]) + months(3), 
               ymin = -22, ymax = 22, alpha = 0.4, fill = pal[3])+
      # annotate('point', x = as.Date(tdatplot$StartDateWindow[4]) - months(3), y = -20, size = 3)+
      annotate('point', x = as.Date(tdatplot$StartDateWindow[4]) + months(3), y = -17, size = dotsize)+    
      annotate('point', x = as.Date(tdatplot$EndDateWindow[4]), y = 3.5, size = dotsize)+
      # annotate('point', x = as.Date(tdatplot$recov12months[4]), y = 7.25, size = 3)+
        # annotate('text', label = '1', x = as.Date(tdatplot$StartDateWindow[4]) - months(3), y = -18, size = 6)+
      annotate('text', label = '2a', x = as.Date(tdatplot$StartDateWindow[4]) + months(3), y = -13, size = textsize)+    
      annotate('text', label = '2b', x = as.Date(tdatplot$EndDateWindow[4]), y = 7.5, size = textsize)+
      # annotate('text', label = '3', x = as.Date(tdatplot$recov12months[4]), y = 9.25, size = 6)+
      scale_x_date(minor_breaks = date_breaks(width = "1 year"))+
      lims(y = c(-22.5, 22.5))+
      theme(plot.margin = unit(c(0, 5, 0, 0.5), 'line'))+
      theme(axis.text.y = element_text(size = base_size, hjust=1),
