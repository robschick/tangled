...
hplotsub1 <- hplot[hplot$date >= as.Date(tdatplot$StartDateWindow[2]) & hplot$date <= as.Date(tdatplot$recov12months[2]), ]
hplotsub2 <- hplot[hplot$date >= as.Date(tdatplot$StartDateWindow[4]) & hplot$date <= as.Date(tdatplot$recov12months[4]), ]
hplotsub1$event <- 2
hplotsub2$event <- 4
...
```{r}
p3 <- ggplot(hplotsub, aes(x = date, y = hanom, fill = pos)) +
      geom_bar(stat = 'identity', position = 'identity', width = 40)+
      labs(x = 'Year', y = '')+
      guides(fill = 'none')+
      scale_x_date(minor_breaks = date_breaks(width = "1 year"))+
