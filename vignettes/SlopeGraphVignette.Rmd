...
p <- ggplot(data = hdat)+
  ...
  geom_segment(data = hsum, aes(y = smed, yend = emed, x = 0, xend = 1), lwd = 1.5) + 
  geom_segment(data = hsum, aes(y = emed, yend = rmed, x = 1, xend = 2), lwd = 1.5) + 
...
p2 <- ggplot(hsuml, aes(x = gearlab3, y = value, fill = variable)) +
  ...
  coord_flip()+
  labs(x = '', y = 'Health Deviation', fill = c('Time Period'))+
  scale_fill_discrete(labels = c('Entanglement\nWindow', 'Recovery\nWindow'))+
  theme_bw()
p2
```
