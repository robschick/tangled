...
```{r}
pregnant$severity <- factor(pregnant$severity, levels = c('unentangled', 'minor', 'moderate', 'severe')) # this should have an added level to account for never yet entangled, i.e. unimpacted
pregnant <- pregnant %>% 
...
Not really - they are all pretty much the same. Parsimony would favor the simpler model, which is model 1:
...
Next we turn to the years since entanglement to test whether or not a similar pattern is seen with these disturbances. First, we prep the data
```{r}
dsub <- pregnant %>% 
  # dplyr::slice_head(n = 250) %>%
  select(EGNo, Year, Pregnant) %>% 
