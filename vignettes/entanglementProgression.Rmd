...
Then, for any female, we find the month of her first calf, and using some indexing, we set all her health values to NA following her first calf. **AMY** we should discuss if this is exactly what we want. The way I've written this code, it removes the health of entangled reproductively active females from all categories - not just the unentangled one. 
...
# Pull Out Health
That's the state/fate vectors complete for each individual animal, and now we want to use those in conjunction with the health data to gauge how health of different classes of animals changes.

To get the fate data married with the health data, we:

1. loop over the columns in that health matrix (n.b. that each column corresponds to a specific month)
