...
So we have those data, i.e. the ones without a valid start date, but we need to do a bit of data processing to calculate the extra variables need. This is because in previous steps, we'd discarded these events. In particular, we need to format the end date properly, and get the firstCalf index.
...
Next we have to add in the first calf index for the reproductively active females. 
...
```{r}
tangledNew <- tangleAll %>% 
  select(-firstCalf) %>% 
  bind_rows(tangled, tangleAll)
