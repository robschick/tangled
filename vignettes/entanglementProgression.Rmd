...
for (i in 1:ncol(health)){
  df <- data.frame(id = canAn, health = health[, i], fate = fate[, i], dateTime = dateVec[i])
  dfSummary <- df %>% 
    group_by(fate) %>% 
...
plot(unentangled$dateTime, unentangled$mean, type = 'l', ylim = c(0, 100), xlim = c(as.Date('1980-01-01'), as.Date("2014-07-01")),
