...
text(0.5, 82, 'Entanglement Window')
text(1.5, 82, 'Recovery Window')
```
...
barplot(df$health, beside = TRUE, space = c(0, 0, 0.2, 0), names = c('MN_During', 'MN_Recov', 'SG_During', 'SG_Recov'),
        col = c("#D95F02", "#1B9E77"), ylab = 'Health Anomaly', border = NA)
```
...
```{r}
library(gridExtra)
pOut <- plotSlopeHealth(dfout, dfn, dfMed, dfAsuml)
pOut
ggsave(plot = pOut, filename = 'print.pdf', path = '/Users/rob/Dropbox/Papers/KnowltonEtAl_Entanglement/images/ggTest', device = 'pdf')
ggsave(plot = pOut, filename = 'web.png', path = '/Users/rob/Dropbox/Papers/KnowltonEtAl_Entanglement/images/ggTest', device = 'png', dpi = 300, width = 9, height = 6, units = 'in')
```
