...
Ok, let's examine a few different formulations. First we'll test if the curves for males and females are significantly different:
...
```{r}
cfit1 <- coxph(Surv(futime, event) ~ interaction(Severity, gender), data = dfSurv)
cfit1
```

Let's look at the relative death rates for each gender/severity combination. We do this by extracting the coefficient vector. Females with minor entanglements are the reference case, so their rate is 1.0. 

```{r}
cmean <- matrix(c(0, coef(cfit1)), nrow=2, byrow = T)
