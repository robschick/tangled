...
`cfit2` suggests this interaction term is significant, and therefore worth keeping in the model. We can verify this by conducting a Likelihood ratio test between these two models:
...
Let's look at the relative death rates for each gender/severity combination. We do this by extracting the coefficient vector. Females with minor entanglements are the reference case, so their rate is 1.0. Essentially we are calculating the hazard ratio, and then using the coefficients for both the males and females to calculate the ratio of gender.

```{r}
cmean <- matrix(c(0, coef(cfit2)), nrow=2, byrow = T)
cmean <- rbind(cmean, cmean[1, ] - cmean[2,])
dimnames(cmean) <- list(c("F", "M", "F/M ratio"), c('Minor', 'Moderate', 'Severe'))
signif(exp(cmean),3)
```

This shows a few interesting findings:

1. Females with severe entanglements are 4 times as likely to die as those with a minor entanglement. 
2. For all categories, females are more likely to die, than males, though the difference is strongest in the Minor category
3. Females and Males in the Moderate category have reduced rates as compared to the Minor category